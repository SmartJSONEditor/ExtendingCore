...
namespace AudioKitCore {
    ...
    float VoiceManager::velToLevel(int midiVelocity)
    {
    ...
    void VoiceManager::play(unsigned noteNumber, unsigned velocity, float noteHz)
    {
        ...
        if (pVoice)
        {
            ...
            pVoice->start(eventCounter, noteNumber, velocity, noteHz, velToLevel(velocity));
        ...
        if (pStalestVoiceInRelease != 0)
        {
            ...
            pStalestVoiceInRelease->restart(eventCounter, noteNumber, velocity, noteHz, velToLevel(velocity));
        }
        else
        {
            // No notes in release phase: restart the "stalest" one we could find
            pStalestVoiceOfAll->restart(eventCounter, noteNumber, velocity, noteHz, velToLevel(velocity));
        }
    }
